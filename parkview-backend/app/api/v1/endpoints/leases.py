...
def get_lease_dashboard_summary(db: Session = Depends(deps.get_db)):
    ...
    current_reference_date = date.today()
    ...
    unit_options = [{"id": u.id, "label": f"{u.unit_number} — {u.floor} {u.unit_type}"} for u in vacant_units]
...
def create_lease(
    ...
    active_lease_exists = db.query(models.Lease).filter(
        models.Lease.unit_id == lease_in.unit_id,
    ...
    if active_lease_exists:
        raise HTTPException(
            ...
            detail= f"Unit {unit.unit_number} is already occupied under an active lease."
        )
